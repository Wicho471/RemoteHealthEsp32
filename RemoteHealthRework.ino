...
void setup() {
  ...
  Logger::enable(true);
  bool test = prefs.load<bool>(KEY_TEST, false);
  Logger::setTest(test);
  Logger::log("Modo simulacion %s\n", (test ? "habilitado" : "deshabilitado")); 


  btSrv.begin("Remote-Health");
  wifi.begin();
  sensorMgr.init();
  //webSrv.begin();
  wsSrv.begin();
...
void loop() {
  ...
  if (now - lastSendTime >= 5000 && wsSrv.getConnectedClients() > 0) {
    ...
    sensorData.toPayload(buffer, BUFFER_SIZE);
    
    if (!String(buffer).endsWith(",NR,NR,NR,NR,NR")) {
